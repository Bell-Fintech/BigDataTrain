...
<script type="text/javascript">
	...
	function mygoods(id) {
		$.jBox("iframe:${ctx}/salecommodityAction.do?method=toEditCommodity&id="+id, {
			title : "编辑货架",
			width : 750,
			height : 500,
			buttons : { /*'关闭': true*/}
    ...
    function deleteById(id,url){
        ...
        var url = "${ctx}/salecommodityAction.do?method=doDelete&id="+id+"&url="+url;
        $.ajax({
            url:url,    //请求的url地址
...
<script type="text/javascript">
	function history() {
		$.jBox("iframe:${ctx}/my/toAddItems.shtml", {
			title : "添加商品",
			width : 750,
			height : 550,
			buttons : { /*'关闭': true*/}
	...
	<div class="bg_color1">
		<div class="pagebody1">
			<div class="wid">
				<ul class="ht1">
					<li class="big3"><a
						href="${ctx}/salecommodityAction.do?method=myList">我的货架</a></li>
					<li class="big4"><a
						href="${ctx}/salecollectionAction.do?method=myCollection">我的收藏</a></li>
					<li class="small">共<span>${page.totalRows}</span>个商品
					</li>
					<li><button class="new1" onclick="history();"
							style="cursor: pointer;">新增货架</button></li>
				</ul>
				<div class="clear"></div>
			</div>
			<div class="wid">
					...
					<div class="wid2">
						<ul id="demo">
							<c:forEach var="items" items="${page.list}">
								<li>
									<dt
										onclick="<c:if test="${empty items.title}">history('${userFlag}');</c:if><c:if test="${not empty items.title}">productgroupedit('${items.id}');</c:if>">
										<a href="#"> <c:if test="${not empty items.url}">
												<img src="${items.url}" />
											</c:if> <c:if test="${empty items.url}">
												<img src="${ctx}/images/tu007.png" />
											</c:if>
										</a>
									</dt> <c:if test="${not empty items.title}">
										<dd>
											<h3>
												<a class="a2"
													onclick="deleteById('${items.id}','${items.url}')" href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
												<a class="a1" onclick="mygoods('${items.id}');" href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
												<span title="${items.title}" style="cursor: pointer;">

													<c:set var="title" value="${items.title}" /> <c:choose>
											...
											<p>
												<span title="${items.content}" style="cursor: pointer;">
													...
													<c:set var="content" value="${items.content}" /> <c:choose>
				...
				<div class="scrollcon mar3">
											...
											<li>
												<p class="p1"
													...
													<a href="javascript:;"><c:if
															test="${not empty items.url}">
															<img src="${items.url}" />
														</c:if></a>


													<c:if test="${empty items.url}">
														<img src="${ctx}/images/tu007.png" />
													</c:if>

												</p> <c:if test="${not empty items.title}">
													<p class="p2">
														<span><a class="a2"
															onclick="deleteById('${items.id}','${items.url}')"
															href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a> <a class="a1"
															onclick="mygoods('${items.id}');" href="#">&nbsp;&nbsp;&nbsp;&nbsp;</a>
															<span title="${items.title}" style="cursor: pointer;">
														...
														</span></span> <i><span title="${items.content}"
															style="cursor: pointer;"> <c:set var="content"
																	value="${items.content}" /> <c:choose>
																...
																</c:choose></span></i> <b><a><fmt:formatDate value="${items.editTime}"
					...
					<ul class="page" style="padding-top: 50px">
						<pg:pager url="${ctx}/my/myItems.shtml" items="${totalRows}"
